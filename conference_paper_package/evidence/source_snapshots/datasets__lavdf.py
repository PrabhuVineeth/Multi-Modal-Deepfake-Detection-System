...
class LAVDFDataset(BaseDeepfakeDataset):
    ...
    def _load_samples(self) -> None:
        ...
        if not loaded_any:
            logger.warning(f"LAV-DF annotations not found under {self.root_dir}/annotations. Falling back to directory scan.")
    ...
    def _add_entry(self, entry: Dict[str, Any]) -> None:
        ...
        if entry_split == "dev":
            entry_split = "val"
        elif not entry_split:
            entry_split = self.split
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if segments:
            ...
            result["boundary_tags"] = torch.full((frame_count,), TFBD_TAGS["REAL"], dtype=torch.long)
        return result
