        ...
        try:
            ...
            for i, detections in enumerate(result.face_detections):
                if detections:
                    ...
                    # No face detected — fallback to using entire frame (resized) and fixed mouth region
                    import cv2
                    fallback_face = cv2.resize(result.frames[i], self.config.face_crop_size)
                    # Extract mouth from a fixed lower-middle coordinates (64 to 160, 114 to 210)
                    fallback_mouth = fallback_face[114:210, 64:160]
                    fallback_mouth = cv2.resize(fallback_mouth, self.config.mouth_roi_size)
                    result.face_crops.append(fallback_face)
                    result.mouth_rois.append(fallback_mouth)
