...
Examples:
  ...
  python demo.py --video sample.mp4 --checkpoint best_model.pth --no-heatmap
    ...
    parser.add_argument(
        ...
        choices=["fakeavceleb", "faceforensics", "lavdf"],
        help="Dataset name for routing: 'fakeavceleb', 'faceforensics', or 'lavdf'",
    )
    parser.add_argument(
    ...
    pipeline.load_model(checkpoint, dataset=args.dataset)
    ...
    cls_label = "[REAL]" if report.classification == "REAL" else "[FAKE]"
    print(f"  Classification: {report.classification} {cls_label}")
