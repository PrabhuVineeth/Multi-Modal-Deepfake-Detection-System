...
\begin{tikzpicture}[scale=0.85]
    ...
    % Draw labels
    \node at (-1.8, 3) {\textbf{Actual REAL}};
    \node at (-1.8, 1) {\textbf{Actual FAKE}};
    \node at (1, 4.4) {\textbf{Predicted REAL}};
    \node at (3, 4.4) {\textbf{Predicted FAKE}};
    
    % Draw cells
    \node[align=center] at (1,3) {\textbf{TN} \\ 2383 \\ (65.4\%)};
    \node[align=center] at (3,3) {\textbf{FP} \\ 1261 \\ (34.6\%)};
    \node[align=center] at (1,1) {\textbf{FN} \\ 697 \\ (19.1\%)};
    \node[align=center] at (3,1) {\textbf{TP} \\ 2947 \\ (80.9\%)};
\end{tikzpicture}
\caption{Confusion Matrix of the routed MDDS model on the LAV-DF test set under the validation-calibrated threshold ($\tau = 0.40$).}
\label{fig:confusion_matrix_lavdf}
\end{figure}

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.49\linewidth]{verdict_dashboard_main.png}
\hfill
\includegraphics[width=0.49\linewidth]{verdict_dashboard_timeline.png}
\caption{Real user interface dashboard of the MDDS deployment. Left: Top panel showing video preview, overall verdict, and normalized component anomaly scores (lip sync, visual identity, temporal continuity, and audio-video synchronization). Right: Bottom panel displaying the pre-frame timeline and localized boundary detections.}
\label{fig:mdds_dashboard_ui}
\end{figure*}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{modality_ablation_results.png}
\caption{Ablation performance comparisons showing AUC/F1-score gains from joint multi-modal fusion vs. single-modality channels.}
\label{fig:ablation_results_chart}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{system_performance_charts.png}
\caption{Validation system metrics including Expected Calibration Error (ECE), training curves, and threshold distribution profiles across FakeAVCeleb and LAV-DF splits.}
\label{fig:system_performance_charts}
\end{figure}
